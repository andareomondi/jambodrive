...
import { LogOut, Settings, Calendar, MapPin, DollarSign, Car as CarIcon, Plus } from 'lucide-react'
        ...
        <Card className="p-6 shadow-sm mb-8">
          <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center">
            <div className="relative w-24 h-24 rounded-full overflow-hidden flex-shrink-0">
              <Image src={currentUser.profileImage} alt={currentUser.name} fill className="object-cover" />
            </div>
            ...
            <div className="text-right">
              <div className="text-3xl font-bold text-accent">{currentUser.totalBookings}</div>
          ...
          {activeBookings.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {activeBookings.map((booking) => (
                <Card key={booking.id} className="overflow-hidden shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-1">
                  <div className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <h3 className="font-semibold text-lg text-foreground">{booking.carName}</h3>
          ...
          {pastBookings.length > 0 ? (
            <div className="space-y-3">
              {pastBookings.map((booking) => (
                <Card key={booking.id} className="p-4 shadow-sm transition-all duration-300 hover:shadow-md">
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="font-semibold text-foreground">{booking.carName}</h3>
                        <BadgeStatus status={booking.status} className="text-xs" />
                      </div>
            ...
            <Card className="p-8 text-center">
              <p className="text-muted-foreground">No completed or cancelled bookings yet</p>
            </Card>
          )}
        </div>
      </div>
