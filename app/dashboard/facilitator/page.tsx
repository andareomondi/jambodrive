  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (data.role === "facilitator" || data.role === "admin") {
          setIsFacilitator(true);
          setBookings(
            data.bookings.filter((b: Booking) => b.status === "confirmed"),
          );
        }
      } catch {
...
${customNotes || "None."}
    ...
    // ── Has fees: same pattern as booking page ─────────────────────────────
    ...
    try {
      // 1. Subscribe to realtime BEFORE firing STK push — mirrors booking page exactly
      ...
      // 2. Fire the STK push — stores facilitator_checkout_id on booking row
      ...
      // 3. Show waiting UI — realtime handles the rest
      setPaymentState("waiting_for_pin");

      // 4. 90s safety timeout — same as booking page
      setTimeout(() => {
  ...
  // ── Payment Modal — copied from booking page ───────────────────────────────
  const PaymentModal = () => {
