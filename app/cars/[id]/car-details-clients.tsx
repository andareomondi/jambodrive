...
// 1. Unified client instantiation tools
    ...
    return (
      <div className="min-h-screen flex flex-col bg-background">
        <div className="flex-1 flex items-center justify-center">
          <EmptyState
            title="Car Not Found"
            description="The car you're looking for doesn't exist inside our tracking database."
            action={{ label: "Back to Fleet Catalog", href: "/cars" }}
          />
        </div>
        <Footer />
      </div>
            ...
            {isFullscreen && (
              <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-4 md:p-8">
                <button
                  ...
                  className="absolute top-4 right-4 md:top-8 md:right-8 text-white hover:text-accent z-50 bg-black/50 rounded-full p-2"
                >
                  <X className="w-8 h-8" />
                </button>
                <div
          ...
          <div className="lg:col-span-1">
            <Card className="p-6 shadow-medium sticky top-32">
              ...
              <div className="space-y-2 mb-6">
                {[
                  { icon: Shield, label: "Full Comprehensive Insurance" },
                  { icon: Calendar, label: "Flexible Extensions Available" },
                  { icon: MapPin, label: "Airport Dropoffs / Pickups" },
                ].map((item, i) => {
              ...
              <div className="space-y-3">
                {car.available ? (
                  <Button
                    asChild
                    className="w-full bg-accent hover:bg-accent/90 text-base py-6 rounded-xl"
                  >
                    <Link href={`/booking/${car.id}`}>Secure with M-Pesa</Link>
                  </Button>
                ) : (
                  <Button
