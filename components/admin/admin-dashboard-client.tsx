...
import {
  Users, Banknote, Calendar, Car as CarIcon, Search, Plus,
  Edit, Trash2, MessageCircle, Filter, ChevronLeft, ChevronRight,
  ChevronDown, ChevronUp, ArrowRight, AlertCircle, RotateCcw, Receipt,
  ClipboardCheck,
} from "lucide-react";
...
function StatusBadge({ status }: { status: string | null }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${STATUS_STYLES[s] ?? STATUS_STYLES.pending}`}>
      {s}
    </span>
  );
}
...
export function AdminDashboardClient({
  ...
  const totalFailed    = useMemo(() => bookings.filter((b) => b.status === "failed").length, [bookings]);
  const totalCustomers = useMemo(() => profiles.filter((p) => p.role === "customer").length, [profiles]);
  ...
  const bookingsPagination = usePagination(filteredBookings, BOOKINGS_PAGE_SIZE);
  const profilesPagination  = usePagination(profiles, USERS_PAGE_SIZE);
  const fleetPagination     = usePagination(cars, FLEET_PAGE_SIZE);

  // ── Refresh helpers ───────────────────────────────────────────────────────

  const refreshCars     = useCallback(() => fetchAdminCars().then(setCars).catch((e) => toast.error(e.message)), []);
  const refreshBookings = useCallback(() => fetchAdminBookings().then(setBookings).catch((e) => toast.error(e.message)), []);
  const refreshProfiles  = useCallback(() => fetchAdminProfiles().then(setProfiles).catch((e) => toast.error(e.message)), []);

  // ── Mutations ─────────────────────────────────────────────────────────────

  const handleBookingStatus = useCallback(async (id: string, status: BookingStatus) => {
    try {
      ...
      if (carId) {
        ...
        setCars((prev) => prev.map((c) => c.id === carId ? { ...c, available } : c));
      }
  ...
  const handleRoleChange = useCallback(async (userId: string, role: string) => {
    try {
      ...
      setProfiles((prev) => prev.map((u) => (u.id === userId ? { ...u, role: role as Profile["role"] } : u)));
  ...
  const handleAddCar  = () => { setSelectedCar(null); setCarModalOpen(true); };
  const handleEditCar = (car: Car) => { setSelectedCar(car); setCarModalOpen(true); };
  const openDeleteConfirm = (car: Car) => { setCarToDelete(car); setDeleteModalOpen(true); };
    ...
    <div className="min-h-screen bg-background">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Manage your fleet, users, and bookings.</p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
          {[
            ...
            { icon: Banknote, label: "Base Revenue", value: `Ksh ${totalRevenue.toLocaleString()}` },
            { icon: Receipt,  label: "Extra Fees",   value: `Ksh ${totalAdditionalFees.toLocaleString()}` },
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-foreground">
                  {bookings.filter((b) => b.status === "confirmed").length} confirmed booking{bookings.filter((b) => b.status === "confirmed").length !== 1 ? "s" : ""} ready for return
                </p>
                <p className="text-xs text-muted-foreground">
                  Use the facilitator tool to process vehicle inspections and returns
                </p>
              </div>
            </div>
        ...
        <Card className="border-border shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] text-sm">
              <thead className="bg-muted/30 border-b border-border">
                <tr>
                  {["ID", "Vehicle", "Customer", "Dates", "Amount", "Status", "Actions"].map((h) => (
                    <th key={h} className="text-left font-semibold text-foreground px-4 py-3 first:pl-6 last:pr-6 whitespace-nowrap">
                      {h}
                    </th>
                  ))}
                </tr>
                  ...
                  bookingsPagination.paged.map((booking) => (
                    <React.Fragment key={booking.id}>
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-foreground whitespace-nowrap">{booking.cars?.name ?? "—"}</td>
                        <td className="px-4 py-3 text-foreground whitespace-nowrap">{booking.profiles?.full_name ?? "Walk-in"}</td>
                        <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">
                          {new Date(booking.pickup_date).toLocaleDateString("en-KE", { month: "short", day: "numeric" })} →{" "}
                          {new Date(booking.return_date).toLocaleDateString("en-KE", { month: "short", day: "numeric" })}
                        </td>
                        ...
                        <td className="px-4 py-3 pr-6 whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                            ...
                            {booking.status === "confirmed" && (
                              <>
                                <Button variant="outline" size="sm"
                                  ...
                                  onClick={() => {
                                    const phone = booking.profiles?.phone ?? booking.mpesa_phone;
                                    if (!phone) { toast.error("No phone number for this booking."); return; }
                                    openWhatsApp(phone,
                                ...
                                <Link href="/facilitator">
                                  <Button size="sm" variant="outline"
                                    ...
                                    <ClipboardCheck className="h-3.5 w-3.5" /> Return
                                  </Button>
                                </Link>
                              </>
                            )}
                            {booking.status === "failed" && (
                              <>
                                {booking.payment_failure_reason && (
                                  <span title={booking.payment_failure_reason}
                                ...
                                <Button size="sm" variant="ghost"
                                  ...
                                  onClick={() => handleDismissFailed(booking.id)}>
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 p-4">
                                ...
                                <div className="rounded-xl border border-border bg-card p-4 space-y-2.5">
                                  <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                                    <Calendar className="h-3.5 w-3.5" /> Logistics
                                  </p>
                                  <div className="space-y-1.5 text-sm">
                                    {[
                                      ["Pickup", booking.pickup_location],
                                      ["Return", booking.return_location],
                                      ["Days",   booking.days ? `${booking.days} days` : "—"],
                                ...
                                <div className="rounded-xl border border-border bg-card p-4 space-y-2.5">
                                  <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                                    <Banknote className="h-3.5 w-3.5" /> Initial Payment
                                  </p>
                                  <div className="space-y-1.5 text-sm">
                                    {[
                                      ["MPesa Receipt", booking.mpesa_receipt_number],
                                      ["MPesa Phone",   booking.mpesa_phone],
                                      ["Amount Paid",   booking.paid_amount ? `Ksh ${Number(booking.paid_amount).toLocaleString()}` : null],
                                    ].map(([k, v]) => (
                                      <p key={k as string} className="text-muted-foreground">
                                        <span className="font-medium text-foreground">{k}: </span>
                                        {v ?? <span className="italic text-muted-foreground/60">N/A</span>}
                                      </p>
                                    ))}
                                  </div>
                                </div>
                                ...
                                <div className="rounded-xl border border-border bg-card p-4 space-y-2.5 sm:col-span-2 xl:col-span-1">
                                  <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                                    <Receipt className="h-3.5 w-3.5" /> Extra Fees
                                  </p>
                                  {booking.additional_fee_amount ? (
                                    <div className={`rounded-lg border p-3 text-sm space-y-1 ${
                                      booking.additional_fee_status === "confirmed"
                                        ? "bg-accent/10 border-accent/30"
                                        : "bg-muted/50 border-border"
                                    }`}>
                                      <p className="font-medium text-foreground">
                                        Ksh {Number(booking.additional_fee_amount).toLocaleString()} collected
                                      </p>
                                      <p className="text-xs text-muted-foreground">
                                      ...
                                      <p className="text-xs text-muted-foreground">
                                        ...
                                        {booking.additional_fee_receipt ?? <span className="italic">Pending</span>}
                                      </p>
                                      {booking.additional_fee_reason && (
                                        <p className="text-xs text-destructive pt-1 border-t border-destructive/20">
                                          <span className="font-medium">Error: </span>{booking.additional_fee_reason}
                                        </p>
                                      )}
                                    </div>
                                  ) : (
                                    <p className="text-sm text-muted-foreground italic">No extra return fees recorded.</p>
                                  )}
                                </div>
                              </div>
                            </div>
          ...
          <Pagination
            page={bookingsPagination.page} totalPages={bookingsPagination.totalPages}
            onPageChange={bookingsPagination.setPage}
            totalItems={filteredBookings.length} pageSize={BOOKINGS_PAGE_SIZE}
          />
        </Card>
        ...
        <Card className="border-border shadow-sm overflow-hidden">
                ...
                <div key={u.id}
                  ...
                  <div className="flex items-center gap-4 w-full sm:w-auto">
                    <div className="relative w-12 h-12 rounded-full overflow-hidden shrink-0 bg-muted flex items-center justify-center">
                      {u.profile_image ? (
                        <Image src={u.profile_image} alt={u.full_name ?? ""} fill className="object-cover" />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1 flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-xs text-muted-foreground flex items-center gap-1">
                          <Calendar className="w-3 h-3" /> {u.total_bookings ?? 0} trips
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-row sm:flex-col gap-2 w-full sm:w-auto sm:ml-auto mt-2 sm:mt-0">
                    <Button size="sm"
                      ...
                      onClick={() => {
                        if (!u.phone) { toast.error("No phone number on file."); return; }
                        openWhatsApp(u.phone, "Hello, this is the admin from Cosmara.");
                      }}>
          ...
          <Pagination
            page={profilesPagination.page} totalPages={profilesPagination.totalPages}
            onPageChange={profilesPagination.setPage}
            totalItems={profiles.length} pageSize={USERS_PAGE_SIZE}
          />
        </Card>
        ...
        <Card className="border-border shadow-sm overflow-hidden">
                  ...
                  <div className="p-5">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <p className="font-bold text-accent text-sm">
                        Ksh {car.price.toLocaleString()}<span className="text-xs text-muted-foreground font-normal">/d</span>
                      </p>
                    </div>
          ...
          <Pagination
            page={fleetPagination.page} totalPages={fleetPagination.totalPages}
            onPageChange={fleetPagination.setPage}
            totalItems={cars.length} pageSize={FLEET_PAGE_SIZE}
          />
        </Card>
      </main>
      ...
      <DeleteCarModal
        ...
        onClose={() => { setDeleteModalOpen(false); setCarToDelete(null); }}
      ...
      <BookingModal
        ...
        onSuccess={() => { refreshCars(); refreshBookings(); }}
      />
    </div>
  );
}