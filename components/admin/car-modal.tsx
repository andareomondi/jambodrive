...
export function CarModal({ open, onOpenChange, car, onSuccess }: CarModalProps) {
  ...
  const { register, handleSubmit, reset, control, formState: { errors } } =
    useForm<CarFormData>({
      defaultValues: {
        name: "", model: "", year: CURRENT_YEAR, price: 0,
        type: "economy", seats: 5, transmission: "automatic",
        fuel: "petrol", fuel_consumption: "", features: "",
  ...
  return (
    <Dialog.Root open={open} onOpenChange={(next) => { if (!next) handleCancel(); }}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-[650px] max-h-[90vh] -translate-x-1/2 -translate-y-1/2 flex flex-col rounded-2xl border border-border bg-card shadow-xl overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95">
            ...
            <form id="car-form" onSubmit={handleSubmit(onSubmit)} className="space-y-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input id="name" required placeholder="e.g. Mercedes"
                    className="rounded-xl bg-muted/40 border-border h-11"
                    {...register("name", { required: true })} />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="model" className="text-xs font-semibold">Model Variant</Label>
                  <Input id="model" required placeholder="e.g. C-Class"
                    className="rounded-xl bg-muted/40 border-border h-11"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <SelectField label="Category"    name="type"         control={control} options={CAR_TYPE_LABELS} />
                <SelectField label="Gearbox"     name="transmission" control={control} options={TRANSMISSION_LABELS} />
                <SelectField label="Fuel"        name="fuel"         control={control} options={FUEL_LABELS} />
              </div>
              <div className="space-y-1.5 mt-4">
                <Label className="text-xs font-semibold">Fuel Consumption</Label>
                <Input placeholder="e.g. 8.5L/100km"
                  className="rounded-xl bg-muted/40 border-border h-11"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <Label className="text-xs font-semibold flex items-center justify-between">
                    Main Photo
                    {coverFile && (
                      <span className="text-[10px] text-accent font-bold uppercase">Selected</span>
                    )}
                  </Label>
                  <div className="relative group cursor-pointer">
                    ...
                    <div className={cn(
                      ...
                      coverFile ? "bg-accent/5 border-accent/40" : "hover:bg-muted/40"
                    )}>
                ...
                <div className="space-y-2">
                  <Label className="text-xs font-semibold flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-muted-foreground uppercase">
                      {galleryFiles.length} file{galleryFiles.length !== 1 ? "s" : ""}
                    </span>
                  </Label>
                  <div className="relative group cursor-pointer">
                    ...
                    <div className={cn(
                      "h-24 rounded-xl border-2 border-dashed border-border flex flex-col items-center justify-center gap-1 transition-colors",
                      galleryFiles.length > 0 ? "bg-accent/5 border-accent/40" : "hover:bg-muted/40"
                    )}>
                      <Plus className="w-5 h-5 text-muted-foreground" />
                      <span className="text-[10px] text-muted-foreground font-medium">
                        {galleryFiles.length > 0 ? `${galleryFiles.length} selected` : "Add gallery photos"}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-1.5 mt-4">
                ...
                <Input placeholder="Bluetooth, Sunroof, AC..."
                  className="rounded-xl bg-muted/40 border-border h-11"
                  {...register("features")} />
              </div>
          ...
          <div className="p-4 border-t border-border bg-background flex gap-3 shrink-0">
            ...
            <Button type="submit" form="car-form" disabled={isLoading}
